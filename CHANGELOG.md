# 更新日志

本项目从 `v1.3.0` 开始记录自定义版本。`v1.0`、`v1.1` 和 `v1.2`
是仓库继承的上游历史标签，不包含当前完整的自定义功能。

## [v2.6.0] - 2026-09-26

- 移动普通框、OBB 和多选框时统一限制在图片边界内；可从图片外开始绘框，完成后自动吸附到图片边缘。
- `R` 显示/隐藏选中的框；未选中时切换全部框。`T` 和 `N` 保留批量隐藏/显示旋转框与普通框。`Ctrl+Shift+L` 显示/隐藏选中框标签；未选中时切换全部标签。
- 多框框选允许从图片外的空白画布开始。
- `Help` 新增使用指南，依次介绍快捷键、基础使用和进阶使用。
- 新增 `Settings` 菜单，集中管理标签快捷键、标签格式、界面语言、自动保存、框标签显示和角点显示。
- 新增英文、简体中文、日文、西班牙文、阿拉伯文（RTL）、法文、韩文界面选项，并记住上次选择。
- `Open Dir` 与 `Open Annotation Dir` 改用不同图标，便于区分。

## [v2.5.3] - 2026-09-25

- 修复 `Ctrl+Shift+L` 虽能开启“Paint Labels”，却只绘制通常为空的 Extra Info、看不到框类别名的问题；现在显示类别名，并在有 Extra Info 时一并显示。
- 切换标签文字或修改框类别后立即刷新画布，靠近图片顶边的标签文字也会留在画布内。

## [v2.5.2] - 2026-09-25

- 修复 v2.5.1 Windows 安装包启动时可能出现的 `No module named 'pkg_resources.extern'` 错误；改用单一 Python 环境打包并验证安装版真正打开主窗口，而不是只判断进程仍在运行。
- 安装升级时，如桌面已有 LabelImg2 Custom 快捷方式，会重新写入指向新版 EXE 的快捷方式和图标；没有快捷方式的用户仍可自行选择是否创建。
- v2.5.1 的界面适配和 Windows 原生文件选择窗口继续保留。建议已下载 v2.5.1 的用户升级到 v2.5.2。

## [v2.5.1] - 2026-09-25

- 按屏幕可用尺寸调整主窗口和类别/快捷键设置窗口的初始大小；窄窗口下顶部工具栏自动分行，避免“置信度”“框型”和缩放文字被裁切。
- 文件与文件夹选择统一使用 Windows 原生资源管理器样式；选择类别文件仍只显示 `.txt`，图片和标签目录选择不变。

## [v2.5.0] - 2026-09-23

- 新增可安装的 64 位 Windows EXE 安装包，内含 Python、PyQt5、CPU 版 PyTorch、Ultralytics 等运行依赖；安装后无需配置 Conda 或 venv。安装版设置保存在用户 AppData，避免安装目录写入权限问题。
- 工具栏增加“框型：普通框 / OBB”选择器；`E` 根据当前选择开始画框，再按一次退出画框状态，默认仍选 OBB。
- 原有工具栏普通框和 OBB 一键画框按钮保留；点击按钮也会同步选择相应框型，之后按 `E` 继续使用该类型。
- 类别下拉框编辑期间 `E` 仍用于按首字母选类，不会触发画框。

## [v2.4.0] - 2026-09-23

### 新增：中文拼音选类、类别编辑快捷键和待确认图片

- 双击类别进入下拉框后，中文类别可以按拼音连续输入（例如 `huolongguo` 查找“火龙果”），也支持拼音首字母；相同字母连续按仍按常用顺序轮换。
- 类别下拉框内可直接按已设置的类别快捷键切换当前框类别，不会进入画框状态。
- File List 右键可标记或取消“待确认”图片；标记呈淡红色，并随刷新和重启保留。
- 修复右键点击 File List 图片时，列表先自动居中滚动、以及菜单可能固定弹到屏幕角落的问题；右键菜单现在以鼠标实际位置弹出，键盘切图仍会居中。
- 画布、File List 和类别下拉框不再接受中文输入法的组合输入，中文输入法开启时快捷键可直接交给 LabelImg2；其他文字输入控件仍可正常输入中文。

### 调整：图片切换方向键

- 图片切换方向键由 `← / →` 改为 `↑ / ↓`，`A / D` 保持不变；连续按键切图后 `File List` 继续保持焦点和居中位置。
- 画布中已选中框时，四个方向键都用于将框向对应方向微调 1 像素；没有选中框时，只有 `↑ / ↓` 切换图片，`← / →` 不执行切图。

### 新增：刷新图片和标签文件夹

- 右侧 `File List` 顶部新增刷新按钮，也可以按 `F5`，一次重新扫描当前图片文件夹与标签文件夹。
- 新增、移除或在外部修改的图片和标签会更新到列表及画布；保留仍存在的当前图片、自然排序和已有绿色确认状态。
- 标签数量与格式继续在后台分批读取，大型数据集刷新时不会同步打开全部 XML/TXT；刷新后当前图片仍保持在列表中间。

### 新增：File List 删除图片并同步删除标签

- 右侧 `File List` 选中图片后可以右键选择“删除图片及对应标签”，也可以直接按 `Del`；切图加载完成后会保留列表焦点，解决按键被画布抢走的问题。
- 使用上/下方向键或 `A`/`D` 切换图片后，焦点自动落到当前图片所在的 `File List`，无需再用鼠标选中即可紧接着按 `Del` 移入回收站。
- 鼠标或键盘切换图片时，当前高亮图片会自动滚动到 `File List` 可视区域的垂直中间，连续查看大量图片时视线位置保持稳定（列表首尾受滚动边界限制）。
- 右键操作会列出图片和对应标签并要求二次确认；`Del` 按键不弹确认、直接操作。图片和同名 `.xml`、`.txt` 全部移入系统回收站，不再永久删除，并且不受当前 Annotation Format 影响。
- `Del` 只在 `File List` 获得焦点时删除图片，在画布和 `Box Labels` 中仍用于删除选中的框。
- 移入回收站后立即从列表移除该图、自动打开相邻图片并刷新标签统计；图片移动失败时不会改动标签文件。

### 改进：框角点更容易选中

- 普通框和 OBB 的角点从 8 像素放大到 12 像素，鼠标悬停时显示约 21 像素的高亮方块。
- 角点鼠标命中半径从 7 像素提高到 12 像素，并按画布缩放反向换算，图片放大或缩小时都保持一致的屏幕点击范围。

## [v2.3.3] - 2026-09-18

### 修复：大型项目选择标签目录时界面卡死

- `Open Annotation Dir` 选择包含大量标签的目录后，右侧图片列表立即显示，不再在界面线程一次性打开全部 XML/TXT。
- 标签数量和标签格式改为事件循环分批读取，每轮最多处理少量文件并主动让出界面控制权；扫描期间仍可切图、画框或操作菜单。
- 切换标签目录时复用现有图片列表，不再重新遍历整个图片目录；状态栏显示读取进度，完成后自动更新项目标签总数和识别到的格式。
- 新扫描会自动替换未完成的旧扫描，关闭程序时也会安全停止，不留下线程或退出崩溃风险。
- labels 文件夹选择器采用只显示目录的轻量模式，避免 Windows 原生选择器读取同目录中上万个标签文件的信息。

### 新增：可切换的类别文件

- `File` 菜单和右侧 `Box Labels` 面板均增加“选择类别文件”入口，无需再手动覆盖 `data/predefined_classes.txt`。
- 选择前显示类别文件完整路径、类别总数以及从 0 开始的类别 ID/名称预览；支持 UTF-8（含 BOM）和常见中文编码。
- 自动保存当前类别文件和最近 12 个历史路径，重启后恢复上次选择，可在不同项目的 `class.txt` 之间快速切换。
- 切换类别文件不会修改当前图片已有框；不属于新预设的现有框类别会临时保留，避免保存时丢失类别。
- 新类别文件会同步更新默认类别和标签快捷键；指向已不存在类别的快捷键会自动移除。
- 类别文件浏览改用只显示 TXT 的轻量列表选择器，禁用图片缩略图和自定义图标扫描；大型图片目录中选择 `class.txt` 不再卡死，应用类别时也不会重扫图片或标签。

### 修复：混合标签格式产生同名双文件

- 打开或自动恢复标签目录时扫描实际文件格式，单一格式会自动覆盖上一次项目残留的全局格式设置。
- 同名 XML/TXT 并存时优先读取修改时间较新的版本；新标签安全写入后清理同名旧格式副本。
- 手动批量转换与模型覆盖/追加标注在目标文件成功写入后统一清理旧格式，避免两份标签继续分叉。
- 混合格式目录会明确提示当前采用的格式；无法识别的 TXT 保留原文件并给出警告。

### 修复：Windows 打包版本读取

- `setup.py` 明确以 UTF-8 读取项目说明，避免中文 Windows 使用 GBK 时执行打包命令报编码错误。

## [v2.3.2] - 2026-08-26

### 修复：模型标注后绿色确认状态

- 模型标注、格式转换等操作刷新文件列表时，按图片完整路径保留已有的绿色确认状态；不同目录中的同名图片不会串用状态。

### 修复：文件列表自然排序

- 图片按相对路径从左到右分段比较：数字段按整数值排序，字母段忽略大小写按字母顺序排序，避免带前导零和长编号的文件名错序。

### 修复：类别首字母选择

- 类别下拉框处于编辑状态时临时禁用 `E` 画 OBB 快捷键，输入 `e` 只会选择以该字母开头的预设类别；退出编辑后自动恢复画框快捷键。

## [v2.3.1] - 2026-08-19

### 新增：自动标注置信度调节

- 在上方工具栏的模型标注按钮旁增加置信度数值框，范围 `0.01–1.00`，默认 `0.25`，步长 `0.05`。
- 批量“自动标注”和“标注当前图”共用当前数值，并把它直接传给 Ultralytics `conf` 参数。
- 修改后的置信度保存在本机设置中并跨重启恢复；非法旧值自动回退到 `0.25`。
- 推理期间锁定数值框，状态栏和完成窗口显示本次实际使用的置信度。

## [v2.3.0] - 2026-08-19

### 新增：当前图片空标签

- 上方工具栏增加“生成空标签”，按当前 `Annotation Format` 为正在查看的图片立即生成空标签。
- Pascal VOC 模式生成无 object 的 XML；YOLO 与 YOLO OBB 模式生成零字节 TXT。
- 当前图片已有框时先询问是否清空，取消不会修改数据；保存失败会自动恢复原框。
- 成功后右侧文件列表显示 `[BG]`，并可通过 `Ctrl+Z` 恢复清空前的框。

### 新增：自定义标签快捷键

- 在右侧 `Box Labels` 中增加“标签快捷键设置”，可配置多个“按键 → 预设类别”映射。
- 目标类别只能从启动时加载的 `class.txt` 中选择，不能写入临时或未知类别。
- 保存前会检查重复映射、菜单/工具栏快捷键及画布操作键，存在冲突时拒绝保存。
- 按下自定义快捷键会选择对应默认类别并直接进入一次 OBB 绘制；完成后不再弹出类别选择器。
- 映射保存在本机设置中并跨重启恢复；类别编辑器打开时会暂时禁用，避免输入时误触。

### 新增：标签数量统计面板

- 在右侧 `Box Labels` 下方显示项目总标签数、当前图片标签数和本次工作已打标签数。
- 项目与当前图片数量会随新建、复制粘贴、删除、撤销、保存及模型标注实时更新。
- 本次工作数量只统计当前程序运行期间仍然存在的新增框；删除本次新增框时同步减少，删除旧框不会误减，撤销删除后恢复。
- 会话来源标记只保存在运行内存中，不写入 XML/TXT，重新启动后计数归零。
- 复用右侧文件列表已有的标签数量缓存，不会因每次修改都重新扫描全部标签文件。

### 改进：切换格式时转换已有标签

- 手动选择新的 `Annotation Format` 时，自动扫描当前图片数据集对应的全部标签。
- 已有标签会批量转换成 Pascal VOC XML、YOLO 或 YOLO OBB，并显示文件级进度条。
- 每个目标文件安全写入成功后才移除旧格式；失败文件保留原标签并在完成窗口列出。
- YOLO 与 YOLO OBB 共用 `.txt` 扩展名，切换时会自动把 5 列与 9 列内容互相转换。
- 没有标签时直接切换后续保存格式并提示修改成功，不显示无意义的进度条。
- 模型自动识别输出类型时只改变模型输出格式，不触发已有人工标签的批量转换。

### 移除：Export to

- 删除 `File > Export to` 菜单以及旧的 XML 单向导出实现。
- 统一使用 `File > Annotation Format` 完成 XML、YOLO、YOLO OBB 双向批量转换。

### 改进：默认开启自动保存

- 新用户首次启动时，`View > Auto Saving` 默认勾选，切换图片会自动保存当前标签。
- 用户手动关闭自动保存后，后续启动仍会尊重已保存的关闭状态。

### 新增：剪切 Box

- 新增标准快捷键 `Ctrl+X`，可一次剪切当前选中的一个或多个 Box。
- 剪切后的第一次粘贴保留类别、大小、角度和原始坐标；也可切换图片后原位置粘贴。
- 剪切和粘贴分别记为一步操作，可使用 `Ctrl+Z` 逐步撤销。
- 新增多框剪切、原位置粘贴和撤销回归测试。

### 改进：单张模型标注可撤销

- 单张自动标注的覆盖或追加结果现在会作为一条独立的 `Ctrl+Z` 撤销记录。
- 模型推理前已有的撤销历史会在图片重新加载后继续保留。
- 推理失败或没有改变标签时不会增加空撤销记录。
- 撤销后按 `Ctrl+S` 或使用自动保存，将恢复结果写回标签文件。
- 新增覆盖、追加和失败场景的撤销回归测试。

## [v2.2.0] - 2026-08-11

### 新增：框操作撤销

- 新增标准快捷键 `Ctrl+Z`，保留 `Ctrl+V` 作为粘贴。
- 支持撤销框移动、粘贴、删除、复制、缩放、旋转以及类别修改等操作。
- 一次连续拖动合并为一步；每张图片独立保留最多 50 步，切换图片后自动清空。
- 新增移动、粘贴、多框删除和连续撤销自动化测试。

### 修复：自动标注类别匹配

- 模型类别与项目预设完全同名时，强制优先映射到该同名类别。
- 修复共享单词被错误计算为 `100%` 相似的问题，例如不再把模型的 `pipe_row` 映射成 `Drill_pipe`。
- 新增当前全部预设类别必须映射回自身的回归测试。

## [v2.1.0] - 2026-08-11

### 新增：当前图片自动标注

- 顶部工具栏和 `File` 菜单新增“标注当前图”。
- 只对当前打开的图片执行一次本地 YOLO / YOLO OBB 模型推理。
- 当前图片已有标签时，明确询问“覆盖原标签”“直接添加”或“取消”。
- 覆盖模式只保存模型新框；追加模式保留当前框并加入模型新框。
- 继续自动识别普通检测与 OBB 模型、切换 YOLO 保存格式并执行类别模糊匹配。
- 单张任务沿用状态栏进度和中止按钮，完成后自动重新加载当前图片。
- 新增覆盖、追加、后台保存和工具栏入口自动化测试。

## [v2.0.2] - 2026-08-11

### 独立仓库发布

- GitHub 仓库正式脱离上游 Fork 网络，作为独立维护的派生项目发布。
- 完整保留上游提交历史、来源说明、原作者版权和 MIT 许可证文本。
- 项目整体继续采用 GNU AGPL v3.0 免费开源，不改变 v2.0.1 的功能行为。
- README、NOTICE、下载链接和软件版本号统一更新为 v2.0.2。

## [v2.0.1] - 2026-08-11

### 许可证合规调整

- 当前组合发行版的整体许可证由 MIT 调整为 GNU AGPL v3.0，以匹配
  Ultralytics 自动标注依赖的开源许可要求。
- 根目录 `LICENSE` 替换为完整 GNU AGPL v3.0 许可证文本。
- 上游 LabelImg2 的原始 MIT 许可证和版权声明原样保存在
  `LICENSE-MIT-UPSTREAM`，不改变上游代码原有权利和归属。
- README、NOTICE、安装教程、软件版本号和打包元数据统一更新。
- 此版本只调整许可证和发布说明，不改变 `v2.0.0` 的自动标注行为。

## [v2.0.0] - 2026-08-11

### 新增：本地模型自动标注

- 工具栏和 `File` 菜单增加“自动标注”。
- 支持加载本地 Ultralytics YOLO 检测或 YOLO OBB `.pt` 模型。
- 模型在后台线程逐张推理，界面状态栏显示文件名、框数量和总体进度。
- 状态栏提供“中止”按钮；中止请求会在当前图片推理完成后生效。
- 自动识别模型任务：普通检测模型保存为 5 列 YOLO，OBB 模型自动切换并保存为
  9 列 YOLO OBB。
- 使用模糊匹配把模型类别映射到项目类别预设，例如 `apple -> apples`、
  `org -> oranges_`、`melon -> name_melon`。
- 完成窗口显示模型类别到项目类别的映射和相似度，方便人工复核。
- 自动标注保留图片目录的相对子目录结构，并写入当前 `Open Annotation Dir`。
- 已存在 XML 或 TXT 标签的图片自动跳过，不覆盖人工标注。
- `.pt` 文件加入 Git 忽略列表；仓库和发行源码包不包含任何第三方模型权重。
- 增加 `ultralytics>=8.3` 运行依赖；未安装时不影响手工标注界面启动。

### 验证

- 通过类别模糊匹配、YOLO OBB 四点坐标转换、后台批量保存、标签重新载入、
  已有标签保护和中止流程测试。
- 通过 Python 语法编译及 Conda 启动器检查。

## [v1.5.0] - 2026-08-10

- 重写 Windows 启动器，支持项目 `.venv`、名为 `labelimg2` 的 Conda 环境和
  当前激活的 Conda 环境。
- 增加 `--venv`、`--conda` 和 `--check` 参数，各环境互不回退和污染。
- 移除对 Windows `py/pyw` 注册表启动路径的依赖。
- 完善中文小白安装教程和首次运行检查。

## [v1.4.0] - 2026-08-10

- 支持直接选择保存 Pascal VOC XML、普通 YOLO 或 YOLO OBB。
- 合并标签读取和保存目录入口为 `Open Annotation Dir`。
- 自动识别 5 列 YOLO 与 9 列 YOLO OBB TXT。
- 支持图片和标签按相同子目录结构递归匹配。
- XML 导出只转换同名 TXT，不复制图片、不划分数据集、不生成 YAML。

## [v1.3.0] - 2026-08-09

- 建立以 OBB 为主的快捷标注流程。
- 增加绘制状态切换、画完自动选择类别、常用类别优先、角度快捷键等功能。
- 增加框复制粘贴、多选、集体移动、滚轮缩放框和跨图片原位置粘贴。
- 增加自然数字排序、目录与标注进度恢复。

## 历史上游标签

- `v1.2`：上游历史版本。
- `v1.1`：上游历史版本。
- `v1.0`：上游历史版本。

[v2.6.0]: https://github.com/auto-sun/labelImg2-custom/releases/tag/v2.6.0
[v2.5.3]: https://github.com/auto-sun/labelImg2-custom/releases/tag/v2.5.3
[v2.5.2]: https://github.com/auto-sun/labelImg2-custom/releases/tag/v2.5.2
[v2.5.1]: https://github.com/auto-sun/labelImg2-custom/releases/tag/v2.5.1
[v2.5.0]: https://github.com/auto-sun/labelImg2-custom/releases/tag/v2.5.0
[v2.4.0]: https://github.com/auto-sun/labelImg2-custom/tree/v2.4.0
[v2.3.3]: https://github.com/auto-sun/labelImg2-custom/tree/v2.3.3
[v2.3.2]: https://github.com/auto-sun/labelImg2-custom/tree/v2.3.2
[v2.3.1]: https://github.com/auto-sun/labelImg2-custom/tree/v2.3.1
[v2.3.0]: https://github.com/auto-sun/labelImg2-custom/tree/v2.3.0
[v2.2.0]: https://github.com/auto-sun/labelImg2-custom/tree/v2.2.0
[v2.1.0]: https://github.com/auto-sun/labelImg2-custom/tree/v2.1.0
[v2.0.2]: https://github.com/auto-sun/labelImg2-custom/tree/v2.0.2
[v2.0.1]: https://github.com/auto-sun/labelImg2-custom/tree/v2.0.1
[v2.0.0]: https://github.com/auto-sun/labelImg2-custom/tree/v2.0.0
[v1.5.0]: https://github.com/auto-sun/labelImg2-custom/archive/refs/tags/v1.5.0.zip
[v1.4.0]: https://github.com/auto-sun/labelImg2-custom/archive/refs/tags/v1.4.0.zip
[v1.3.0]: https://github.com/auto-sun/labelImg2-custom/archive/refs/tags/v1.3.0.zip
